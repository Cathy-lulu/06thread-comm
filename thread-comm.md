...
[Inter-thread Communication](#inter-thread-commuication)
  * [Mailbox](#mailbox)
    * [Mailbox Working Mechanism](#mailbox-working-mechanism)
    * [Mailbox Control Block](#mailbox-control-block)
    * [Management of Mailbox](#management-of-mailbox)
      * [Create and Delete Mailbox](#create-and-delete-mailbox)
      * [Initialize and Detach Mailbox](#initialize-and-detach-mailbox)
      * [Send Mail](#send-mail)
      * [Send Mails with Waiting](#send-mails-with-waiting)
      * [Receive Mails](#receive-mails)
     * [Mail Usage Sample](#mail-usage-sample)
     * [Occasions to Use Mails](#occasions-to-use-mails)
   * [Message Queue](#message-queue)
     * [Message Queue Working Mechanism](#message-queue-working-mechanism)
     * [Message Queue Control Block](#message-queue-control-block)
     * [Management of Message Queue](#management-of-message-queue)
       * [Create and Delete Message Queue](#create-and-delete-message-queue)
       * [Initialize and Detach Message Queue](#initialize-and-detach-message-queue)
       * [Send Message](#send-message)
       * [Send Message with Waiting](#send-message-with-waiting)
       * [Receive Message](#receive-message)
     * [Message Queue Application Example](#message-queue-application-example)
     * [Occasions to Use Message Queue](#occasions-to-use-message-queue)
       * [Sending Messages](#sending-messages)
       * [Synchronizing Messages](#synchronizing-messages)
   * [Signal](#signal)
     * [Signal Working Mechanism](#signal-working-mechanism)
     * [Management of Signals](#management-of-signals)
     ...
     * [Signal Application Example](#signal-application-example)
...
#### Send Message with waiting

Users can also send messages to specified message queue through the following function interface:

![image-20191218113937647](C:\Users\RT-Thread_User\AppData\Roaming\Typora\typora-user-images\image-20191218113937647.png)

The difference between rt_mb_send_wait() and rt_mb_send() is that there is waiting time. If the message queue is full, the thread sending the message will wait according to the set timeout parameter. If by the set timeout, there is still no available space, then the thread sending the message will wake up and return an error code. The following table describes the input parameters and return values for this function:

Input parameters and return values of rt_mb_send_wait()

![image-20191218114717791](C:\Users\RT-Thread_User\AppData\Roaming\Typora\typora-user-images\image-20191218114717791.png)
...
Wait for the arrival of the set signal. If this signal did not arrive, suspend the thread until the signal arrival or wait until time exceeds the specified timeout. If the signal arrived, the pointer pointing to the signal body is stored in si, as follows is the function to wait for signal. 
